...
LANG='javascript'
CUDA_VISIBLE_DEVICES=0,2 python main_codesearch.py -lang $LANG --gpus 2 -ptm $PTM --scheduler linear \
-ckpt $BASE_CKPT/$MODEL/best_model.ckpt --output_dir $EXP_NAME -bs 16 --prefix
CUDA_VISIBLE_DEVICES=0 python eval_codesearch.py --is_multitask -ckpt $CKPT_PATH/$EXP_NAME/$LANG/$MODEL/best_model.ckpt -lang $LANG -ptm $PTM --prefix

LANG='go'
CUDA_VISIBLE_DEVICES=0,2 python main_codesearch.py -lang $LANG --gpus 2 -ptm $PTM  \
-ckpt $BASE_CKPT/$MODEL/best_model.ckpt --output_dir $EXP_NAME -bs 16 --prefix
CUDA_VISIBLE_DEVICES=0 python eval_codesearch.py --is_multitask -ckpt $CKPT_PATH/$EXP_NAME/$LANG/$MODEL/best_model.ckpt -lang $LANG -ptm $PTM --prefix
